...
# --- 1. CONFIG DASHBOARD ---
st.set_page_config(page_title="Executive Pastel Dashboard", layout="wide")
...
st.markdown("""
    ...
    /* Warna Pastel untuk Metric Delta */
    [data-testid="stMetricDelta"] svg { fill: #A5D6A7 !important; }
    
    .main-title { color: #475569; font-weight: 800; font-size: 2.5rem; margin-bottom: 20px; }
    </style>
...
with col_right:
    ...
    fig_clust = px.scatter(df_filtered, x='units_sold', y='profit', color='segment_cluster',
                           color_discrete_sequence=["#FFB7B2", "#B2CEE0", "#FDFD96"], # Pastel Pink, Blue, Yellow
...
with c2:
    ...
    fig_bar = px.bar(df_filtered.groupby('product')['profit'].sum().reset_index(), 
                     x='product', y='profit', color='product',
